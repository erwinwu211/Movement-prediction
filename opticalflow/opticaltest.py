...
cap = cv2.VideoCapture("soccer3.mp4")
...
while(cap.isOpened()):
    ...
    good_old = p0#[st==1]
    # draw the tracks
    f = open('output/frame_'+str(frame_n)+'.txt', 'w') 
	...
	if dis > 5 and dis < 100 :
	    flow=cv2.line(flow,(a,b),(c,d),[0,0,255],2)
	    frame=cv2.line(frame,(a,b),(c,d),[0,0,255],2)
	if degree != 0:
	    f.writelines(str(i)+": degree="+str(degree)+", distance="+str(dis)+"  \n")
